...
contract RehypothecationHooks is BaseHook {
    ...
    // Encrypted tick storage for privacy - keeping Fhenix for this
    ...
    mapping(bytes32 => ebool) private encryptedWithdrawalNeeded; // Keep for privacy
    mapping(bytes32 => euint32) private encryptedTimestamp; // Keep for privacy

    // Owner
    address public owner;

    // Events - keeping encrypted events for privacy
    ...
    event EncryptedRebalancingDecision(bytes32 indexed positionKey, bytes encryptedDecision);
    event EncryptedPostSwapCompleted(bytes32 indexed positionKey, bytes encryptedResult);
     ...
     */
    constructor(IPoolManager _poolManager, IAave _aavePool, ILiquidityOrchestrator _liquidityOrchestrator)
    ...
    function _decryptHookData(bytes calldata hookData) internal returns (int24 lowerTick, int24 upperTick) {
        ...
        if (isEncrypted) {
            ...
            (InEuint32 memory encLowerTick, InEuint32 memory encUpperTick) =
                abi.decode(encryptedData, (InEuint32, InEuint32));
            ...
            console.log("Decrypted ticks:", lowerTick);
            console.log("Decrypted ticks:", upperTick);
        } else {
            // Decode plain data
    ...
    function _afterAddLiquidity(
        ...
        _handleAfterAddLiquidity(key, params, delta, lowerTick, upperTick, currentTick, positionKey);
    ...
    function _handleAfterAddLiquidity(
        ...
        if (liquidityOrchestrator.isPositionExists(positionKey)) {
            _updateExistingPosition(positionKey, currentTick, amount0In, amount1In, asset0, asset1);
        } else {
            _createNewPosition(
                key, lowerTick, upperTick, amount0In, amount1In, positionKey, currentTick, asset0, asset1
            );
        }
    }
    ...
    function _createNewPosition(
        ...
        console.log("Total liquidity for new position:", totalLiquidity);
        console.log("Amount0 in:", amount0In);
        console.log("Amount1 in:", amount1In);

        // Updated PositionData with regular uint256 values
        ...
        console.log("Creating new position with total liquidity:", data.totalLiquidity);
        console.log("Reserve amount0:", data.reserveAmount0);
        console.log("Reserve amount1:", data.reserveAmount1);
        console.log("Aave amount0 (token0):", data.aaveAmount0);
        console.log("Aave amount1 (token1):", data.aaveAmount1);
    ...
    function _afterRemoveLiquidity(
        ...
        (uint256 liqAmount0, uint256 liqAmount1, address asset0, address asset1) =
    ...
    function _computeAfterRemoveAmounts(PoolKey calldata key, bytes32 positionKey, BalanceDelta delta)
        internal
        view
        returns (uint256 liqAmount0, uint256 liqAmount1, address asset0, address asset1)
    {
        uint256 withdrawn0 = delta.amount0() > 0 ? uint256(int256(delta.amount0())) : 0;
        uint256 withdrawn1 = delta.amount1() > 0 ? uint256(int256(delta.amount1())) : 0;

        ILiquidityOrchestrator.PositionData memory pos = liquidityOrchestrator.getPosition(positionKey);

        // Now using regular uint256 values - no decryption needed
        uint256 reserveAmount0 = pos.reserveAmount0;
        uint256 reserveAmount1 = pos.reserveAmount1;

        // Allow for rounding errors: if almost all is withdrawn, treat as zero
        uint256 tolerance = 2; // 2 wei tolerance for rounding
        if ((reserveAmount0 <= withdrawn0 + tolerance) && (reserveAmount1 <= withdrawn1 + tolerance)) {
            liqAmount0 = 0;
            liqAmount1 = 0;
        } else {
            liqAmount0 = reserveAmount0 > withdrawn0 ? reserveAmount0 - withdrawn0 : 0;
            liqAmount1 = reserveAmount1 > withdrawn1 ? reserveAmount1 - withdrawn1 : 0;
        }
    ...
    function _emitEncryptedLiquiditySignal(bytes32 positionKey, int24 currentTick) internal {
        // Keep encrypted events for privacy
        ...
        emit EncryptedLiquiditySignal(positionKey, abi.encode(encryptedNeedsWithdrawal, encryptedTime));
    }

    function _preparePreSwap(bytes32 positionKey, int24 currentTick, address token0, address token1) internal {
    ...
    function _emitEncryptedWithdrawalPrepared(bytes32 positionKey) internal {
        ...
        emit EncryptedWithdrawalPrepared(positionKey, abi.encode(encryptedSuccess));
    }
    ...
    function _afterSwap(
        ...
        _handleSwapStateUpdates(key, params, delta, positionKey, oldTick, newTick, lowerTick, upperTick);
    ...
    function _handleSwapStateUpdates(
        ...
        ILiquidityOrchestrator.PositionData memory position = liquidityOrchestrator.getPosition(positionKey);
        ...
        if (shouldRebalance) {
            _handleRebalancing(key, positionKey, oldTick, newTick, lowerTick, upperTick);
        }
    }
    ...
    function _updatePositionReserves(
        ...
        // Now using regular uint256 values - no decryption needed
        uint256 reserveAmount0 = position.reserveAmount0;
        uint256 reserveAmount1 = position.reserveAmount1;

        if (params.zeroForOne) {
            (reserveAmount0, reserveAmount1) = _updateReservesZeroForOne(reserveAmount0, reserveAmount1, delta);
        } else {
            (reserveAmount0, reserveAmount1) = _updateReservesOneForZero(reserveAmount0, reserveAmount1, delta);
        }

        liquidityOrchestrator.updateReserves(positionKey, reserveAmount0, reserveAmount1);
    }

    function _updateReservesZeroForOne(uint256 reserve0, uint256 reserve1, BalanceDelta delta)
        internal
        pure
    ...
    function _updateReservesOneForZero(uint256 reserve0, uint256 reserve1, BalanceDelta delta)
        internal
        pure
        returns (uint256, uint256)
    {
    ...
    function _handleRebalancing(
        ...
        require(oldTick >= lowerTick && oldTick <= upperTick, "Invalid old tick range");
        require(newTick < lowerTick || newTick > upperTick, "New tick should be out of range");
        ...
        ILiquidityOrchestrator.PositionData memory finalPosition = liquidityOrchestrator.getPosition(positionKey);
        require(
    ...
    function _emitSwapEvents(bytes32 positionKey, int24 oldTick, int24 newTick, bool zeroForOne) internal {
        // Keep encrypted events for privacy
        ...
        emit EncryptedPostSwapCompleted(positionKey, abi.encode(FHE.asEuint32(1)));
    }

    /**
     * @dev Generates a unique position key based on the pool key and ticks
     * @param key The pool key
