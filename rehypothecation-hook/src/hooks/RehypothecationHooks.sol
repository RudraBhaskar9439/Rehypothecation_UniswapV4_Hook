...
contract RehypothecationHooks is BaseHook {
        ...
        returns (uint256 liqAmount0, uint256 liqAmount1, address asset0, address asset1)
    {
        ...
        if ((pos.reserveAmount0 <= withdrawn0 + tolerance) && (pos.reserveAmount1 <= withdrawn1 + tolerance)) {
            liqAmount0 = 0;
            liqAmount1 = 0;
        } else {
            liqAmount0 = pos.reserveAmount0 > withdrawn0 ? pos.reserveAmount0 - withdrawn0 : 0;
            liqAmount1 = pos.reserveAmount1 > withdrawn1 ? pos.reserveAmount1 - withdrawn1 : 0;
        }
    ...
    function _afterSwap(
        ...
        if (params.zeroForOne) {
            ...
            encryptedAmount0Delta = FHE.asEuint32(uint32(amount0In));
            encryptedAmount1Delta = FHE.asEuint32(uint32(amount1Out));
        } else {
            // token1 -> token0
            uint256 amount1In = delta.amount1() < 0 ? uint256(int256(-delta.amount1())) : 0;
            ...
            encryptedAmount0Delta = FHE.asEuint32(uint32(amount0Out));
            encryptedAmount1Delta = FHE.asEuint32(uint32(amount1In));
        }
