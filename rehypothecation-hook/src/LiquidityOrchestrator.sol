...
contract LiquidityOrchestrator is ILiquidityOrchestrator {
    ...
    function preparePreSwapLiquidity(
        ...
        try Aave.withdraw(asset0, type(uint256).max, address(this)) returns (
            uint256 withdrawnAmount0
        ) {
            try
                Aave.withdraw(asset1, type(uint256).max, address(this))
            returns (uint256 withdrawnAmount1) {
                p.reserveAmount1 = FHE.add(
                    p.reserveAmount1,
                    FHE.asEuint256(withdrawnAmount1)
                );
            ...
            p.state = PositionState.IN_RANGE;
            p.reserveAmount0 = FHE.add(
                p.reserveAmount0,
                FHE.asEuint256(withdrawnAmount0)
            );
    ...
    function executePostSwapManagement(
        ...
        FHE.decrypt(p.reserveAmount0);
        FHE.decrypt(p.reserveAmount1);
        ...
        uint256 amount0ToDeposit = (reserveAmount0 * (100 - reservePCT)) /
            100;
        uint256 amount1ToDeposit = (reserveAmount1 * (100 - reservePCT)) /
            100;
        ...
        if (amount0ToDeposit > 0) {
            try Aave.deposit(asset0, amount0ToDeposit, address(this), 0) {
                p.reserveAmount0 = FHE.sub(
                    p.reserveAmount0,
                    FHE.asEuint256(amount0ToDeposit)
                );
                p.aaveAmount0 = FHE.add(
                    p.aaveAmount0,
                    FHE.asEuint256(amount0ToDeposit)
                );
                depositSuccess = true;
        ...
        if (amount1ToDeposit > 0) {
            try Aave.deposit(asset1, amount1ToDeposit, address(this), 0) {
                p.reserveAmount1 = FHE.sub(
                    p.reserveAmount1,
                    FHE.asEuint256(amount1ToDeposit)
                );
                p.aaveAmount1 = FHE.add(
                    p.aaveAmount1,
                    FHE.asEuint256(amount1ToDeposit)
                );
                depositSuccess = true;
    ...
    function preparePositionForWithdrawal(
        ...
        if (p.aaveAmount0.gt(FHE.asEuint256(0))) {
            try
                Aave.withdraw(asset0, type(uint256).max, address(this))
            returns (uint256 withdrawnAmount0) {
                p.reserveAmount0 = FHE.add(
                    p.reserveAmount0,
                    FHE.asEuint256(withdrawnAmount0)
                );
        ...
        if (p.aaveAmount1.gt(FHE.asEuint256(0))) {
            try
                Aave.withdraw(asset1, type(uint256).max, address(this))
            returns (uint256 withdrawnAmount1) {
                p.reserveAmount1 = FHE.add(
    ...
    function handlePostWithdrawalRebalance(
        ...
        p.aaveAmount0 = FHE.asEuint256(0);
        p.aaveAmount1 = FHE.asEuint256(0);
        p.totalLiquidity = FHE.add(FHE.asEuint256(liqAmount0) + FHE.asEuint256(liqAmount1));
        ...
        returns (uint256 amount0, uint256 amount1, PositionState state)
    {
        ...
        FHE.decrypt(FHE.add(p.reserveAmount0 , p.aaveAmount0));
        FHE.decrypt(FHE.add(p.reserveAmount1 , p.aaveAmount1));

        (uint256 Amount0,)=FHE.getDecryptResultSafe(FHE.add(p.reserveAmount0 , p.aaveAmount0));
        (uint256 Amount1,)=FHE.getDecryptResultSafe(FHE.add(p.reserveAmount1 , p.aaveAmount1));
        return (
            Amount0,
            Amount1,
            p.state
